...
class OrdersViewController: UIViewController {
    ...
    override func viewDidLoad() {
		...
		var defaults = UserDefaults.standard
			   db.collection("trips").whereField("phonep",isEqualTo: defaults.string(forKey: "phone")).getDocuments() { (querySnapshot, err) in
				   if let err = err {
					   print("Error getting documents: \(err)")
				   } else {
					for document in querySnapshot!.documents  {
						  let str : Double = document.data()["price"] as! Double
											  self.arr.append(order(name: "User PHone : "+String(document.data()["phoneu"] as! String),phone: ("price : "+String(str)),id: "ORDER ID : "+String(document.documentID.uppercased() as! String),time: document.data()["day"]))
...
extension OrdersViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.date.text = formattedTimeZoneStr
